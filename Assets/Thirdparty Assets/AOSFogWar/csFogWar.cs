    ...
    public class csFogWar : MonoBehaviour {
        ...
        public class FogRevealer {
            ...
            public Vector2Int GetCurrentLevelCoordinates(csFogWar fogWar) {
                // Use fogWar mapping: map revealer world pos to module-space then compute units
                Vector3 modulePos = fogWar.MapWorldToModule(revealerTransform.position);
                return new Vector2Int(
                    fogWar.GetUnitX(modulePos.x),
                    fogWar.GetUnitY(modulePos.z));
            }
        ...
        // --- ADDED: plane mapper slot (assign PlaneMapper_XZ or PlaneMapper_XY) ---
        [Header("Plane Mapping (optional)")]
        [Tooltip("Assign a plane mapper if your game uses a different ground plane than XZ. Use PlaneMapper_XZ (default) or PlaneMapper_XY for XY ground.")]
        [SerializeField]
        private MonoBehaviour planeMapperObject = null; // assign a component that implements IPlaneMapper

        // Helper property to access mapper as the interface (null if not assigned or wrong type)
        private IPlaneMapper PlaneMapper => planeMapperObject as IPlaneMapper;
        ...
        // Map an arbitrary Unity world point into the module's expected space (module uses X,Z ground with Y vertical)
        public Vector3 MapWorldToModule(Vector3 world) {
            if (PlaneMapper != null) return PlaneMapper.MapWorldToModule(world);
            return world; // default identity (XZ)
        }

        // Map a module-space point into Unity world space
        public Vector3 MapModuleToWorld(Vector3 module) {
            if (PlaneMapper != null) return PlaneMapper.MapModuleToWorld(module);
            return module;
        }

        // Map module-space half-extents to world-space half-extents
        public Vector3 MapModuleExtentsToWorld(Vector3 moduleExtents) {
            if (PlaneMapper != null) return PlaneMapper.MapModuleExtentsToWorld(moduleExtents);
            return moduleExtents;
        }

        // Direction in world space to use when module expects Vector3.down
        public Vector3 GetModuleDownDirection() {
            if (PlaneMapper != null) return PlaneMapper.ModuleDownDirectionInWorld();
            return Vector3.down;
        }

        // Get rotation to orient fog plane primitive correctly
        public Quaternion GetPlaneRotation() {
            if (PlaneMapper != null) return PlaneMapper.PlaneRotation();
        ...
        private void InitializeFog() {
            ...
            // module-space center for plane
            Vector3 moduleCenter = new Vector3(
                MapWorldToModule(levelMidPoint.position).x,
                MapWorldToModule(levelMidPoint.position).y + fogPlaneHeight,
                MapWorldToModule(levelMidPoint.position).z);

            fogPlane.transform.position = MapModuleToWorld(moduleCenter);

            fogPlane.transform.rotation = GetPlaneRotation();
        ...
        private void UpdateFog() {
            // Update plane position using module mapping
            Vector3 moduleCenter = new Vector3(
                MapWorldToModule(levelMidPoint.position).x,
                MapWorldToModule(levelMidPoint.position).y + fogPlaneHeight,
                MapWorldToModule(levelMidPoint.position).z);
        ...
        private void ScanLevel() {
            ...
            for (int xIterator = 0; xIterator < levelDimensionX; xIterator++) {
                ...
                for (int yIterator = 0; yIterator < levelDimensionY; yIterator++) {
                    // Build module-space center point
                    float moduleMidY = MapWorldToModule(levelMidPoint.position).y;
                    Vector3 moduleCenter = new Vector3(
                        GetWorldX(xIterator),
                        moduleMidY + rayStartHeight,
                        GetWorldY(yIterator));

                    // Map to real world
                    Vector3 worldCenter = MapModuleToWorld(moduleCenter);

                    Vector3 moduleHalfExtents = new Vector3(
                        (unitScale - scanSpacingPerUnit) / 2.0f,
                        unitScale / 2.0f,
                        (unitScale - scanSpacingPerUnit) / 2.0f);

                    Vector3 worldHalfExtents = MapModuleExtentsToWorld(moduleHalfExtents);

                    bool isObstacleHit = Physics.BoxCast(
                        worldCenter,
                        worldHalfExtents,
                        GetModuleDownDirection(),
                        Quaternion.identity,
                        rayMaxDistance,
                        obstacleLayers,
        ...
        public Vector3 GetWorldVector(Vector2Int worldCoordinates) {
            // Construct module-space vector then map to world
            float moduleMidY = MapWorldToModule(levelMidPoint.position).y;
            Vector3 modulePoint = new Vector3(
                GetWorldX(worldCoordinates.x + (levelDimensionX / 2)),
                moduleMidY,
                GetWorldY(worldCoordinates.y + (levelDimensionY / 2))
            );
            return MapModuleToWorld(modulePoint);
        }
        ...
        public Vector2Int GetUnitVector(Vector3 worldCoordinates) {
            Vector3 module = MapWorldToModule(worldCoordinates);
            return new Vector2Int(GetUnitX(module.x), GetUnitY(module.z));
        }
        ...
        public float GetWorldX(int xValue) {
            float moduleMidX = MapWorldToModule(levelMidPoint.position).x;
            if (levelData.levelDimensionX % 2 == 0) {
                return (moduleMidX - ((levelDimensionX / 2.0f) - xValue) * unitScale);
            }

            return (moduleMidX - ((levelDimensionX / 2.0f) - (xValue + 0.5f)) * unitScale);
        }
        ...
        public int GetUnitX(float xValue) {
            // xValue here is module-space x (MapWorldToModule was called by caller)
            float moduleMidX = MapWorldToModule(levelMidPoint.position).x;
            return Mathf.RoundToInt((xValue - moduleMidX) / unitScale);
        }
        ...
        public float GetWorldY(int yValue) {
            float moduleMidZ = MapWorldToModule(levelMidPoint.position).z;
            if (levelData.levelDimensionY % 2 == 0) {
                return (moduleMidZ - ((levelDimensionY / 2.0f) - yValue) * unitScale);
            }

            return (moduleMidZ - ((levelDimensionY / 2.0f) - (yValue + 0.5f)) * unitScale);
        }
        ...
        public int GetUnitY(float yValue) {
            // yValue here is module-space z (the module's second ground axis)
            float moduleMidZ = MapWorldToModule(levelMidPoint.position).z;
            return Mathf.RoundToInt((yValue - moduleMidZ) / unitScale);
        }
        ...
        private void OnDrawGizmos() {
            ...
            for (int xIterator = 0; xIterator < levelDimensionX; xIterator++) {
                for (int yIterator = 0; yIterator < levelDimensionY; yIterator++) {
                    Vector3 modulePoint = new Vector3(GetWorldX(xIterator),
                                                      MapWorldToModule(levelMidPoint.position).y,
                                                      GetWorldY(yIterator));
                    Vector3 worldPoint = MapModuleToWorld(modulePoint);
                    ...
                    if (levelData[xIterator][yIterator] == LevelColumn.ETileState.Obstacle) {
                        ...
                        Handles.DrawWireCube(
                            worldPoint,
                            MapModuleExtentsToWorld(new Vector3(
                                unitScale - scanSpacingPerUnit,
                                unitScale,
                                unitScale - scanSpacingPerUnit)));
                    } else {
                        Gizmos.color = Color.yellow;
                        Gizmos.DrawSphere(worldPoint, unitScale / 5.0f);
                    }

                    if (shadowcaster.fogField[xIterator][yIterator] == Shadowcaster.LevelColumn.ETileVisibility.Revealed) {
                        Gizmos.color = Color.green;
                        Gizmos.DrawSphere(worldPoint, unitScale / 3.0f);
                    }
                }
            }
        }
