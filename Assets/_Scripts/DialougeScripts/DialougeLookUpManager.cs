    ...
    public Dialog GetDialouge(EnumCharacter character)
    {
        ...
        switch (character)
        {
            case EnumCharacter.LifeGauard:
                ...
                else if (!GameManager.Instance.secondStoryFinished && GameManager.Instance.secondStoryCollected)
                {
                    if (this.isInDebugMode) Debug.Log("Returning second story dialogue");
                    GameManager.Instance.secondStoryFinished = true;
                    return dialogTree.storyDialouges[5];
                }
                else if (!GameManager.Instance.thirdStoryFinished && GameManager.Instance.thirdStoryCollected)
                {
                    if (this.isInDebugMode) Debug.Log("Returning third story dialogue");
                    GameManager.Instance.thirdStoryFinished = true;
                    return dialogTree.storyDialouges[6];
                }
                else if (!GameManager.Instance.fourthStoryFinished && GameManager.Instance.fourthStoryCollected)
                {
                    if (this.isInDebugMode) Debug.Log("Returning fourth story dialogue");
                    GameManager.Instance.fourthStoryFinished = true;
                    return dialogTree.storyDialouges[7];
                }
                else if (!GameManager.Instance.secretDialougeFinished && GameManager.Instance.secretDialougeActive)
                {
                    if (this.isInDebugMode) Debug.Log("Returning secret dialogue");
                    GameManager.Instance.secretDialougeFinished = true;
                    return dialogTree.storyDialouges[8];
                }
                else
                {
